...
export default async function PlanoPage() {
  ...
  // Lifetime e admin → dashboard direto (quem veio pelo Hotmart nunca vê esta página)
  ...
  const isExpired    = profile?.plan_expires_at != null && new Date(profile.plan_expires_at) < new Date();
        ...
        {isBlocked ? (
          ...
          <div className="rounded-2xl border border-brand/30 bg-brand/5 p-5 text-center space-y-1.5">
            <div className="inline-flex items-center gap-1.5 bg-brand text-white text-xs font-bold px-3 py-1 rounded-full mb-1">
              ✨ {daysLeft !== null ? `${daysLeft} dia${daysLeft !== 1 ? "s" : ""} restante${daysLeft !== 1 ? "s" : ""}` : "Teste ativo"}
            </div>
        ...
        <div className="text-center space-y-2 pt-1">
          {!isBlocked && (
            <p className="text-xs text-slate-400">
              Já pagou?{" "}
              <a
                href={waLink(`Olá! Já paguei para assinar o ${NOME_PRODUTO} (${user.email}). Aguardando liberação.`)}
                target="_blank"
                rel="noopener noreferrer"
